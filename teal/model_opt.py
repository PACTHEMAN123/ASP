...
from teal.self_attn_opt import _monkeypatch_self_attn
from teal.mlp_opt import _monkeypatch_fc


def _monkeypatch_layer(layer, path, grabbing_mode=False):
    layer.path = path
    layer.grabbing_mode = grabbing_mode
    layer.fc1 = _monkeypatch_fc(layer.fc1, f"{path}/mlp/fc1", grabbing_mode=grabbing_mode)
    layer.fc2 = _monkeypatch_fc(layer.fc2, f"{path}/mlp/fc2", grabbing_mode=grabbing_mode)
...
class SparseModelMixin:
    ...
    def set_grabbing_mode(self, mode):
        for layer in self.model.decoder.layers:
            layer.fc1.grabbing_mode = mode
            layer.fc2.grabbing_mode = mode
    ...
    def set_apply_prefill(self, apply_prefill):
        for layer in self.model.decoder.layers:
            ...
            for proj in ['fc']:
                layer.fc1.sparse_fns[proj].apply_prefill = apply_prefill
                layer.fc2.sparse_fns[proj].apply_prefill = apply_prefill
    ...
    def build_sparse_layers(self, histogram_path, grab_acts):
        ...
        self.model.decoder.layers = nn.ModuleList(layers)
    ...
    def set_mlp_sparsity(self, sparsity):
        for layer in self.model.decoder.layers:
            layer.fc1.sparse_fns['fc'].set_threshold(sparsity)
            layer.fc2.sparse_fns['fc'].set_threshold(sparsity)
    ...
    def set_sparsities(self, sparsities):
        for proj, sparses in sparsities.items():
            if proj in ['q', 'k', 'v', 'o']:
                ...
                for layer, sparsity in zip(self.model.layers, sparses):
                    layer.fc1.sparse_fns[proj].set_threshold(sparsity)
                    layer.fc2.sparse_fns[proj].set_threshold(sparsity)
