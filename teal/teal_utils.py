...
def get_sparse_model(model_name, device, histogram_path, **kwargs):
    ...
    if device == 'auto':
        # multi gpu
        return SparseModel.from_pretrained(model_name, torch_dtype=torch.float16, device_map="auto", histogram_path=histogram_path, **kwargs)
    else:
        return SparseModel.from_pretrained(model_name, torch_dtype=torch.float16, device_map=device, histogram_path=histogram_path, **kwargs)